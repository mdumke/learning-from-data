...
--[[
  ...
  with a hypthesis set of the form h(x) = a * x^2
--]]

require('helpers')
math.randomseed(os.time())

-- returns a single w optimizing h(x)=wx^2 as hypothesis function
compute_hypothesis = function (x, y)
  local x_train = torch.pow(x, 2)
...
compute_bias_estimate = function (w, n)
  ...
  local y_predict = torch.pow(x_test, 2) * w
...
compute_variance_estimate = function (g_bar, n, d)
  ...
  for i = 1, d do
    ...
    y_g = torch.pow(x_test, 2) * g
    y_g_bar = torch.pow(x_test, 2) * g_bar
...
bias = compute_bias_estimate(g_bar, 10000)
var = compute_variance_estimate(g_bar, 10000, 10000)
